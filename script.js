const questions = [
  ...
  const goToNextQuestion = () => {
      ...
      if (currentQuestionIndex < totalQuestions) {
          ...
          // Mostra il punteggio totale alla fine
          showFinalScore();
      }
  };
  
  const showFinalScore = () => {
    // Cancella il contenuto della sezione domanda e risposta
    ...
    finalScoreSection.className = "final-score-section";
  
    finalScoreSection.innerHTML = `
        <h2>Quiz completato!</h2>
        <p>Il punteggio totale è <strong>${score}</strong> su ${totalQuestions}.</p>
    `;
  
    document.body.appendChild(finalScoreSection);
  };
